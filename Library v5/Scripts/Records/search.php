...
$sql = "SELECT * FROM `books` WHERE books.Book_id LIKE \"%$searchvalue%\" or books.Status LIKE \"%$searchvalue%\"";
$result = mysqli_query($conn, $sql);
if (mysqli_num_rows($result) > 0) {
     echo "<div class='table-responsive'><table id='myTable' class='table table-striped table-bordered'>
             <thead class='thead-dark'><tr><th>BOOK ID</th>
             <th>Book Type</th>
                          <th>STATUS</th>
                          <th>MRP</th>
     ...
     while($row = mysqli_fetch_assoc($result)){
        ...
        echo "<tr><td>".$row['Book_id']."</td><td>".$row['Book_Type']."</td>
